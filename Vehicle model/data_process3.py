...
def dp_process(driving_cycle, driving_cycle_number):
    ...
    # for t in range(T - 2, -1, -1):
        ...
        np.save(f"./dataset/saved_data_total_segments/J{driving_cycle_number}.npy", J)
        np.save(f"./dataset/saved_data_total_segments/policy{driving_cycle_number}.npy", policy)

with open('./dataset/total_segments.pkl', 'rb') as f:
    total_segments = pickle.load(f)


for driving_cycle_number, driving_cycle in tqdm(enumerate(total_segments), total=len(total_segments), desc="Total task", ncols=100, leave=True, dynamic_ncols=True):

    file_path = f"./dataset/saved_data_total_segments/J{driving_cycle_number}.npy"
